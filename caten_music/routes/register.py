...
from flask import Blueprint, render_template, abort, request, redirect, jsonify, url_for
...
ajax_validate_register_bp = Blueprint(
    "ajax_validate_register_bp", __name__, template_folder="templates"
)
...
def register():
    ...
    if request.method == "POST":
        ...
        for k, v in check_result.items():
            if not v:
                return render_template(
                    "error/403.html", error_message="Wrong " + k.replace("_check", "")
                ), 400
        ...
        if checkExist.username(username=username):
            return render_template(
                "error/403.html", error_message="使用者名稱已被註冊"
            ), 400

        # 檢查 email 是否已註冊
        if checkExist.email(email=email):
            return render_template(
                "error/403.html", error_message="電子信箱已被註冊"
            ), 400
        ...
        try:
            return render_template(
                "activation/after_register.html", msg_text="註冊資料已送出"
            ), 201
...
@ajax_validate_register_bp.route(
    "/ajax/validate/register/<username_>/<email_>", methods=["POST"]
)
