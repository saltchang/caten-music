...
activate_account_bp = Blueprint(
    "activate_account_bp", __name__, template_folder="templates"
)

resend_activate_mail_bp = Blueprint(
    "resend_activate_mail_bp", __name__, template_folder="templates"
)

reset_password_bp = Blueprint(
    "reset_password_bp", __name__, template_folder="templates"
)

ajax_validate_email_bp = Blueprint(
    "ajax_validate_email_bp", __name__, template_folder="templates"
)
...
def activate_account(gate, token):
    ...
    if check_result:
        if gate == "account_activate":
            user = UserModel.query.filter_by(id=check_result.get("user_id")).first()
            ...
            return redirect(
                url_for("reset_password_bp.reset_password", step="setting", token=token)
            )
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            return render_template(
                "activation/after_register.html", msg_text="啟動信已成功寄出"
            ), 200
        else:
    ...
    if step == "setting":
        if request.method == "POST":
            ...
            # 當有人故意送出奇怪的request
                ...
                return render_template(
                    "error/403.html", error_message="Don't Play With Me."
                ), 400
            ...
            for k, v in check_result.items():
                if not v:
                    return render_template(
                        "error/403.html",
                        error_message="Wrong " + k.replace("_check", ""),
                    ), 400

            # 如果兩次輸入的密碼不一樣
            if not password == confirm_password:
                return render_template(
                    "error/403.html", error_message="兩次輸入的密碼不一樣，請再試一次"
                ), 400
